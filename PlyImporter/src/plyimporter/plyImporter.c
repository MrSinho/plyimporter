...
void plyLoadFile(const char* path, PlyFileData* ply, PlyLoadFlags flags) {
	...
	ply->vertex_count *= (ply->vertex_stride / 4);
	//BINARY
...
void plyExtractVPositions(PlyFileData* ply) {
	...
	for (uint32_t i = 0; i < ply->vertex_count; i += (ply->vertex_stride / 4)) {
...
void plyExtractUVs(PlyFileData* ply) {
	...
	uint32_t uvCount = 0;
	for (uint32_t i = 6; i < ply->vertex_count; i += (ply->vertex_stride / 4)) {
...
void plyExtractVertexNormals(PlyFileData* ply) {
	...
	for (uint32_t i = 3; i < ply->vertex_count; i += (ply->vertex_stride / 4)) {
